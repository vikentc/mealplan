...
export default function NutritionBadge({ nutrition, scale = 1, servings }: NutritionBadgeProps) {
  ...
  return (
    <div className="space-y-4">
      {/* 1. Primary Macronutrients (Collapsible on Mobile) */}
      <div className="space-y-2">
        <button
          type="button"
          onClick={() => setIsMacrosOpen(!isMacrosOpen)}
          className="w-full md:hidden py-3.5 px-5 bg-amber-100 hover:bg-amber-200 border-3 border-foreground rounded-2xl flex items-center justify-between font-black text-xs uppercase tracking-wider cursor-pointer shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-y-[2px] active:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] transition-all"
        >
      ...
      {/* 2. Detailed Micronutrient Declaration (Less visible & Collapsible) */}
      <div className="border-2 border-dashed border-foreground/35 rounded-2xl p-4 bg-secondary/5">
        <button
          type="button"
          onClick={() => setIsDetailedOpen(!isDetailedOpen)}
          className="w-full flex items-center justify-between text-left font-black text-xs text-foreground/80 uppercase tracking-wider cursor-pointer"
        >
          <span className="flex items-center gap-2">
